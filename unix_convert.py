...
def convert_file(filepath):
    ...
    try:
        ...
        # 写入文件，使用UTF-8（无BOM）和LF行尾
        with open(filepath, 'w', encoding='utf-8', newline='\n') as f:
            f.write(content)
        
        print(f"Converted {filepath}: {src_encoding} -> utf-8(no BOM), LF")
        return True
    except Exception as e:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python unix_convert.py <project_directory>")
        sys.exit(1)
